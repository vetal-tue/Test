...
module tb_sync_fifo_fwft_reg_pow2;
  ...
  localparam FWFT_DELAY = 1; // Задержка падения rd_empty в тактах (1, 2 или 3)
  ...
  task test_2_fwft_specifics();
    ...
    write_word(16'hA5A5);
    repeat (FWFT_DELAY) @(posedge clk);  // Было: @(posedge clk);
    #1;
  ...
  task test_3_boundary_conditions();
    ...
    for (int i = 0; i < DEPTH; i++) begin
      ...
      if (rd_data !== exp_data) begin
        ...
        //        i, exp_data, rd_data);
        $display("ERROR [T3]: Read data mismatch at index %0d! Expected: 0x%0h, Got: 0x%0h", i,
                 exp_data, rd_data);
        err_count++;
      end
  ...
  task test_5_specific_timings();
    ...
    while (!rd_empty) begin
      ...
      if (rd_data !== ref_mem[ref_rd_ptr]) begin
        $display("ERROR [T5 Drain at %0t]: Data mismatch! Exp: 0x%0h, Got: 0x%0h", $time,
                 ref_mem[ref_rd_ptr], rd_data);
        err_count++;
      end
    ...
    if (rd_empty !== 1 || wr_full !== 0) begin
      $display(
          "ERROR [T5 Reset]: Flags did not clear immediately on reset! rd_empty=%b, wr_full=%b",
          rd_empty, wr_full);
      err_count++;
    end
