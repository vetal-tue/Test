module async_fifo_fwft_xilinx_style #(
    parameter DATA_W = 16,
    parameter ADDR_W = 4,
  ...
  function [ADDR_W:0] gray2bin(input [ADDR_W:0] g);
    integer i;
    for (i = 0; i <= ADDR_W; i = i + 1) gray2bin[i] = ^(g >> i);
  endfunction
...
// ALTERA:
  ...
  // (* ram_style = ramstyle *) reg [DATA_W-1:0] ram[0:(1<<ADDR_W)-1];
  (* ram_style = RAM_STYLE *) reg [DATA_W-1:0] ram[0:DEPTH-1];

  always @(posedge wr_clk) if (wr_en && !wr_full) ram[wr_ptr[ADDR_W-1:0]] <= wr_data;
  ...
  (* shreg_extract = "no" *) reg [DATA_W-1:0] stage0_data, stage1_data;
  (* shreg_extract = "no" *) reg stage0_valid, stage1_valid;
  ...
  wire stage1_valid_next = (stage1_valid && !pop1) || push1;
