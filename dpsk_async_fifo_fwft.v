// ============================================================================
// Асинхронное FIFO (First Word Fall Through) с конвейерной задержкой
// Оптимизированная версия: один порт чтения памяти, без масок счётчиков
// ============================================================================
module dpsk_async_fifo_fwft #(
    ...
    // ------------------------------------------------------------------------
    // Память FIFO (распределённая, синхронная запись, асинхронное чтение)
    // ------------------------------------------------------------------------
    ...
    // Вычисление следующего указателя записи
    ...
    // ------------------------------------------------------------------------
    // Счётчик заполнения в домене записи (wr_cnt) – без маски
    // ------------------------------------------------------------------------
    ...
    // ========================================================================
    // ДОМЕН ЧТЕНИЯ (rd_clk) – конвейерная задержка первого слова
    // ========================================================================
    ...
    // Комбинационные данные из памяти (один порт чтения)
    ...
    // ------------------------------------------------------------------------
    // Счётчик заполнения в домене чтения (rd_cnt) – без маски
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // Регистр выходных данных (конвейерная задержка первого слова)
    // ------------------------------------------------------------------------
    always @(posedge rd_clk or posedge rd_rst) begin
        if (rd_rst)
        ...
        // Если FIFO пуст, сохраняем предыдущее значение (rd_empty замаскирует его)
    end
